...
class Solution {
    ...
    private static void findPaths(Node node, int targetSum, ArrayList<Integer> currentPath, ArrayList<ArrayList<Integer>> result) {
        if (node == null)
            return;
        ...
        for (int i = currentPath.size() - 1; i >= 0; i--) {
            ...
            if (currentSum == targetSum && i == 0) {
                result.add(new ArrayList<>(currentPath.subList(i, currentPath.size())));
            }
        }

        findPaths(node.left, targetSum, new ArrayList<>(currentPath), result);
        findPaths(node.right, targetSum, new ArrayList<>(currentPath), result);
    }
}
