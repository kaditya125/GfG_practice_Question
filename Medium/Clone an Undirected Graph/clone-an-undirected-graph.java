...
*/
 class Solution {
    public Node cloneGraph(Node node) {
        ...
        return dfsClone(node, cloneMapping);
    }

    private Node dfsClone(Node originalNode, Map<Node, Node> cloneMapping) {
        if (cloneMapping.containsKey(originalNode)) {
            return cloneMapping.get(originalNode);
        }

        Node cloneNode = new Node(originalNode.val, new ArrayList<>());
        cloneMapping.put(originalNode, cloneNode);

        for (Node neighbor : originalNode.neighbors) {
            cloneNode.neighbors.add(dfsClone(neighbor, cloneMapping));
        }
